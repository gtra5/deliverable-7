...
A browser-based 12x12 tile-map editor built with React 19, Redux Toolkit, and Tailwind CSS v4. Paint, erase, and flood-fill a grid using 20 terrain types, customise every tile's colour with a built-in colour editor, and toggle grid lines on or off — all with full responsive support across desktop, tablet, and mobile.
...
│
├── src/
│   ├── App.jsx                      # Root layout — provider composition, drawer state
...
│   │   ├── Toolbar.jsx              # Sidebar (desktop) / bottom drawer (mobile)
│   │   ├── GridCanvas.jsx           # Painting canvas — pointer events, renders 144 Tile cells
│   │   ├── Tile.jsx                 # Single grid cell — Redux selector, React.memo wrapped
│   │   └── ColorEditorModal.jsx     # Per-tile colour editor — wheel, sliders, HEX/RGB inputs
│   │
│   ├── context/
│   │   └── EditorContext.jsx        # Session UI state — tool, tile type, grid lines, colour overrides
│   │
│   └── redux/
│       ├── store.js                 # Redux store configuration
│       └── gridSlice.js             # Grid matrix — updateTile, fillMap, clearMap reducers
│
├── .gitignore
...
Redux owns the 12x12 tile matrix. This data is:
...
3. **Grid Lines toggle** and **Clear Map** button at the bottom

### `GridCanvas.jsx`

Renders the 12x12 grid using CSS Grid. Handles all pointer events at the container level:
...
- Uses `onMouseEnter`/`onMouseLeave` with `filter: brightness(1.2)` for hover feedback
...
- **Colour wheel** — canvas-drawn HSL wheel; click or drag to pick hue and saturation
...
All defaults can be overridden per-session using the Colour Editor (hover a tile row, click the pencil icon).

---

## Design System — Quantus Palette 2025

All UI colours are defined as CSS custom properties in `index.css` and applied via inline `style` props to guarantee they are never dropped by Tailwind's JIT scanner.
